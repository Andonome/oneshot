...
The PCs will need to spend Story Points to navigate this space, so remind them that they each have 5 Story Points which can be used to declare that they know a language, that they have known another PC, or to find a friend with some specialized skill.
...
As a result, any attack with a weapon which requires 6 or more Initiative inflicts a penalty equal to the number of initiative points over 6 which it requires.
...
\iftoggle{aif}{\footnote{See Adventures in Fenestra, \autoref{magicalitems} \ref{magicalitems} for details on magical items.}}{}}
	\end{itemize}
	\item{Encounter 4}
	\begin{itemize}
...
Remember that if a player cannot succeed in a roll, it's possible for them to take a resting action.%
\iftoggle{core}{%
	\footnote{See the core book, page \pageref{restingactions} for Resting Actions.}
}{}
