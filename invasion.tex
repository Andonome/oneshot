...
Once they escape into the once-gnomish warren, the troupe might:
...
\iftoggle{oneshot}{
  ...
  \pgls{gm} sheet, for recording a few notes about the \glspl{pc} and taking general notes on upcoming encounters, or \glspl{npc}.
...
\iftoggle{hardcore}%
  ...
  Have the player mark $1D6$ \glspl{fatigue} on the character sheet.}

\subsection{Setbacks \& Death}

If any character dies, another can be introduced once the troupe reaches any prison (rooms \ref{entrycell} and \ref{secondPrison}).
The goblins regularly send out raiding parties to capture people, and those people get dumped back into the prisons, so even if the \glspl{pc} have passed a prison, any time they return, more prisoners can be found and liberated.
