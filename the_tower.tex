...
\ifnum\value{temperature}=0%
  ...
  Each character without warm clothing receives \pgls{ep} each \gls{interval}.
\else%
  \ifnum\value{temperature}=3%
  And despite the wood's shade, any travel during the afternoon \gls{interval} inflicts \pgls{ep}.
  \fi%
\fi
...
\begin{exampletext}
  ...
  The tower lay unoccupied, until \gls{alchemist} -- \pgls{warden} who took to the study of alchemy -- set himself up there, along with servants and a hidden store of treasure.
\end{exampletext}
...
The first lies 3 miles to the North West.
The fire hosts a camp of cowardly \glspl{guard} who fled their posts when the goblinoid horde descended.

The second lies 4 miles North, and comes from \gls{alchemist}'s tower.

\paragraph{If the troupe head towards either light,}
they will have to descend into the forest, at which point they can no longer see the lights, making \gls{navigation} difficult.
\setcounter{track}{8}
\ifnum\value{temperature}=0\addtocounter{track}{4}\fi%
Finding the lights requires an \roll{Intelligence}{Survival} roll~(\gls{tn}~\arabic{track}).
Each Failure Margin on the roll adds another mile to the journey, and +1 to the \gls{tn} for future rolls (assuming the troupe haven't arrive after \pgls{interval}).%
\footnote{This is a deathly situation, but every reasonable idea the players have to help get their bearings should reduce the \gls{tn} by 1.}
...
they will divide the \gls{vigil} between the two groups.
Otherwise, the \glspl{guard} will keep \pgls{vigil}, whether or not the \glspl{pc} do so.

Have the party roll \roll{Charisma}{Empathy} (\tn[10]).

\subsubsection{The \Glsentrytext{ogre} Ambush}

If the party spotted and killed the goblins, all is well.
Otherwise, the goblins leave to fetch some \glspl{ogre}, to add the guards to their own soup.
...
requires a \roll{Wits}{Vigilance} roll, against the \glspl{ogre}' \roll{Dexterity}{Stealth} (\tn).

\subsection{The \Glsfmttext{crawler}}

En route to the tower, the party run into \pgls{crawler}'s web.
...
\noindent
Have the party make \pgls{bandAct} of \roll{Wits}{Survival}, \tn[10].
Success indicates that one has spotted goblin footprints nearby.

The characters guess at least a dozen goblins, and half a dozen \glspl{ogre}, from the footprints.

\subsubsection{Burnt \Glsfmtplural{village}}
...
\subsection{Inside the Tower}

\mapentry[towerPortcullis]{The Portcullis}

Two \glspl{ogre} stand inside, happily eating a horse.
They have lowered the gates, so the troupe cannot rush in to attack.

\ogre[\npc{\N\T[2]}{2 \Glsfmtplural{ogre}}]
...
Further goblins hide in here, eating a horse the \glspl{ogre} left for them.

\goblin

\begin{boxtext}
  Outside, a full moon glimpses in through the arrow slits in the walls.
  Goblins have vandalized this armoury, but the armour itself remains undamaged.
...
\footnote{Whenever the \gls{guard} came to the tower, \gls{alchemist} gifted them weapons.
...
\begin{boxtext}
  ...
  At the top, you see an open door, with \pgls{ogre} inside a kitchen, pulling down fruits with one hand and salted meats with the other.
...
The kitchen still holds 8 meals' worth of food, but the \gls{ogre} will eat all of it soon enough.

\paragraph{If the party look formidable but unthreatening,}
the \gls{ogre} leaves them be.
Otherwise, he attacks.

\ogre[\npc{\N\M}{Hungry \Glsentrytext{ogre}}]
...
\begin{boxtext}
  ...
  Scrolls have calculations concerning abstract co\"ordinates, such as `area C', with no mention of what that area may be.
\end{boxtext}
...
\begin{enumerate}
  ...
  \item{A jar with the web-spinning organ of \pgls{crawler}, soaked in wine.}
  ...
  It could make \pgls{ingredient} for a Fate \gls{boon} with a bit of work.}
  \item
...
\ifnum\value{temperature}>0
  ...
  However, \pgls{basilisk} will be upon them soon\ldots
\else%
...
\ifnum\value{temperature}>0%
  ...
  The next \gls{interval}, a nasty stench arrives in the wind -- \pgls{basilisk} has arrived.
