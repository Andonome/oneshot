...
\begin{exampletext}
  ...
  barred the door to the main hall (room \ref{spiral}), then blocked off room \vref{webTrap} by spinning water into webs, to give the appearance of giant, fresh webs, hanging from floor to ceiling.
...
\iftoggle{hardcore}{
  ...
  they can jump down (it's only a couple of steps' distance) and flank the ogres, or jump over them and run towards room \ref{banquet}.
...
If the party want the food,%
\footnote{A recent raiding party managed to get enough food to save, and one of the goblins convinced the \glspl{ogre} to leave it here for later.}
they will have to make a \roll{Dexterity}{Stealth} roll to pilfer it without making any noise (\tn[8]).
...
\begin{itemize}
  ...
  \item
  \Pgls{bow} which deals $1D6+2$ Damage (requires at least Strength +2)
  \item
...
\begin{boxtext}
  In the distance, you can see a massive pool of water shimmering in the darkness with a gentle tide.
  Past the pool, the next room hosts goblins arguing with each other, surrounded by candlelight.
\end{boxtext}

The goblins in room~\vref{duelling} argue over what to do about the \gls{woodspy} in the pool, as it ate one of them already.%
\footnote{A small stream of water from above once provided the gnomes here with a place to bathe and fetch water.
...
the \gls{woodspy} reaches up and try to grab them.

\paragraph{If the \glspl{pc} attempt to shoot at the \gls{woodspy},}
then it retreats into the water, where it remains safe from \glspl{projectile}.
...
Currently the room hosts a rabble of goblins, trapped by the \gls{woodspy} in room \ref{woodspies}.
...
\footnote{When gnomes practice magic, things can get dangerous, so they set aside a room where all the living flames and gateway \glspl{spell} can go off, without people accidentally getting in the way.}
...
\footnote{When gnomes slept here, some of the cheeky young ones wanted to sneak out and play unnoticed, so they poked away at the walls and eventually dug a secret exit for themselves behind a tapestry.}
...
\iftoggle{hardcore}{
  ...
  \begin{boxtext}
    Past the trees and down the hill, the Sunlight shows burning \glspl{village} in the distance, but farther, the great walled town stands strong.
  \end{boxtext}
