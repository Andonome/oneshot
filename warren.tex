...
When the goblinoid hordes entered, Bellcut hid in the side of the room while his companions fought.
...
Unfortunately, he does not speak the goblin language, and everyone soon noticed the problem.

The two druids interrogate Kalama, who remains silently baffled, until the \glspl{pc} or the sounds of a fight interrupt them.

\begin{boxtext}

  As you round the corner, you see two figures, clothed in leather satchels.
...
\goblincaster[\npc{\F\N}{Hunched Goblin Druid}]

\goblincaster[\npc{\M\N}{Snotty Goblin Druid}]
...
\iftoggle{oneshot}{
  ...
  If any say `yes', have them mark off a \glspl{storypoint} (on the back of their character sheet) and explain where they learnt the language.

  Every time players spend a \glspl{storypoint}, the character gains 5 \glspl{xp} for every \glspl{storypoint} marked off so far (so the second gives 2 \glspl{xp}, the third gives 3, and so on).%
...
See \autoref{creaturesAndItems}: \nameref{creaturesAndItems} for details on the calamities which ensue after reading a \gls{deep} Scroll.
...
\iftoggle{oneshot}{
  ...
  if none got an opportunity to spend \glspl{storypoint} on the scrolls (above, in room \ref{spellCasters}).
}{}
...
\paragraph{If they enter the portal to flee from the goblins,}
\iftoggle{hardcore}{%
  they give chase\ldots for a while, however goblins without food tire very quickly.
  If they run through 3 or more areas, the goblins must turn back due to exhaustion.
}{%
  the goblins leave them alone -- their hyperactive metabolism tires too quickly without food.
...
\begin{exampletext}
  ...
  The cautious beast won't approach the goblins and ogres to attack when he has no idea how many there are around him.
  ...
  Similarly, the goblins don't want to take any chances, so they've decided to just leave him where he is.

  The goblins and dragon have ended with a stalemate, so he has taken a nap.
...
\iftoggle{hardcore}{}{%
  Remind the players that they can spend \pgls{storypoint} to say that they know another language, as long as they say when they learnt the language.
}
...
\begin{itemize}
  ...
  \item{Two gem-encrusted shortswords (worth 20 \glspl{sp})}
...
If any of them try to do so, have them roll a \roll{Wits}{Medicine} check to realize their mistake, \tn[8].
...
%!
Since then, the druids morphed those slugs into nura slugs.
...
As with any \gls{deep} Scroll, it leads to the Realm of Shifting Corridors.
...
The goblins do not know these activation words, so they simply keep it under blankets as they find the light irritating.
...
another player should spend 2 \glspl{storypoint} to introduce an old friend.
