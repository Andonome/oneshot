...
\noindent
The \glspl{ogre} captured \gls{sadMan} from a nearby village, along with others.
...
\iftoggle{oneshot}{
  ...
  with a \roll{Dexterity}{Stealth} roll, against the goblin druid's \roll{Wits}{Vigilance} (\tn).
  As before, one player rolls $2D6$, and each \gls{pc} who wants to sneak out adds their own Bonuses to that \gls{natural} -- but if a single \gls{pc} fails, everyone fails.
}{}
...
\iftoggle{oneshot}{%
  As before, the only one player rolls, and other characters use the same \gls{natural} to produce their own result.
}{}
...
\begin{itemize}
  ...
  \footnote{Every 100 coins has \pgls{weight} of 1.}
  \item
  A chest containing 300 \glspl{sp}
  \item
  An ivory short bow (with string, but no arrows)
  \item
  A small backpack (can hold up to \pgls{weight} of 3)
  \item
...
they know to keep quiet and follow, but will only keep quiet if the party succeed on an \roll{Intelligence}{Empathy} roll (\tn[10]).
