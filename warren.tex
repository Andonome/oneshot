...
He explains to them that many prisoners waited here, until the nuramancers%
\iftoggle{hardcore}{}{\footnote{A `nuramancer' is a goblin spellcaster, who uses nuramancy magic.}}
came to turn one into an ogre, who then ate the others, except for Chris, who hid under the debris.
...
{\Dagger, torch\iftoggle{oneshot}{}{, map}}% EQUIPMENT
{}

\widePic{Roch_Hercka/waking}

\iftoggle{hardcore}{
  Alf attacks immediately.
}{
  \paragraph{If anyone tries to fight Alf,}
  he fights!

  \ldots otherwise he sits there looking stupid for a second, but attacks the next round.
}
...
then add 3 Fatigue Points for their time starving in prison, run `\nameref{raidingParty}' (\autopageref{raidingParty}) and throw in some new prisoners.
The players can then spend Story Points to introduce old friends, and try to escape again, with increased numbers.
...
\iftoggle{core}{%
  \footnote{One player rolls 2D6 for the entire group, but each character can add their own bonuses to get a different result.
  See the core rules, page \pageref{grouproll} for more.}
}%
{}
...
\newRule{Damage Resistance}{

  Damage Resistance (DR) reduce Damage taken due to armour, or just a creature's thick hide.
  The nura rat has `{\scshape DR 1}', so it removes 1 from any Damage Taken.
