...
\begin{itemize}
  ...
  \item
  The goblin nuramancers observe for a moment, then flee to the life (room \ref{lift}). 
...
they attempt to flee, gather the other goblins in the kitchen.
...
\begin{itemize}
  ...
  \item
  Anyone with Invocation 2 or more can spend 2 MP to cast a fireball.
\begin{itemize}
  ...
  \item
  Players can spend 1 AP to \textit{Keep Edgy} against missile attacks such as fireballs.
  This allows them to add their Speed + Vigilance Bonus to the roll.
...
\iftoggle{oneshot}{
  \paragraph{Ask the players if they think their characters should be able to speak Gnomish.}
  If any say `yes', ask them to mark off a Story Point, and then explain why they can speak Gnomish, and record that reason on the back of the character sheet.

  Every time players spend a Story Point,%
  \iftoggle{core}{\footnote{See the core rules, \autoref{stories} for more on Story Points.}}{}%
  the character gains 1 XP for every Story Point marked off so far (so the second gives 2 XP, the third gives 3, and so on\ldots)
  
  If anyone can read the runes, they say ``Realm of Bright Rocks''.
}{
...
\iftoggle{oneshot}{% This weapon chart goes next to the kitchen room below.
  \begin{figure*}[t!]
  \begin{nametable}[p{0.2\textwidth}YYYY]{Kitchen Weapons}

  \textbf{Name} & \textbf{Attack Bonus} & \textbf{Damage Bonus} & \textbf{AP Cost} & \textbf{Weight Rating} \\\hline
...
\iftoggle{oneshot}{
  ...
  Two more goblins are sleeping in a corner, making five in total.
}
...
\iftoggle{oneshot}{
  ...
  \paragraph{As the players select a weapon for their characters,}
  have them write down the stats one at a time.
  \begin{itemize}

    \item
    The \textit{Attack Bonus} adds to their Dexterity + Combat roll in combat.
    \item
    The \textit{Damage Bonus} adds to their Damage.
    Anyone with +4 Damage replaces their Damage Bonus with a $D6$, so $1D6+4$ Damage becomes $2D6$, and $1D6+5$ Damage becomes $2D6+1$.
    \item
    ...
    If a weapon's Weight Rating is higher than the character's Strength penalty, then every point over gives them -1 AP at the start of Combat, and +1 Fatigue Points at the end of Combat.
  \end{itemize}
}{
...
\iftoggle{oneshot}{
  Resting a while in the kitchen regenerates a number of Fatigue Points equal to half the player's current HP (rounded up).
