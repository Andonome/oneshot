...
\newRule{Re-Rolls \& Group Rolls}{
  ...
  \paragraph{Example 1:}
  the troupe are sneaking down a hallway, and have to roll Stealth + Dexterity (TN 9).
  One player rolls $2D6$, and gets a `7'.
  Characters with a total bonus of +2 were walking quietly, but if even a single PC had a bonus of +1 or less, they made a noise, and the entire group is given away.
...
\begin{exampletext}
  ...
  Now the PCs have been shunted into the cell with him, he knows his time is short.

\end{exampletext}

Chris explains the situation to the PCs (whether or not anyone asks him).

\paragraph{Trying to plea with Alf}
won't prove easy.
The player can attempt a Charisma + Empathy roll (TN 12), to have him leave the room out of shame and guilt, but he won't help anyone.
...
Convincing Chris to help someone requires a Charisma + Empathy roll (TN 9).
...
\iftoggle{oneshot}{%
  This gives the PCs a bonus to any attack roll equal to their Wits + Vigilance +3 (Alf cannot coordinate well in the dark).
}{}
...
\paragraph{If the PCs manage to throw the human foot towards Alf,}
he stops to eat it for 2 rounds, doing nothing, else unless attacked.

\paragraph{If the PCs try to kill Chris immediately,}
...
\iftoggle{oneshot}{}{
  \paragraph{If the PCs search Blara's body,}
...
She returns some hours later, throws in a new prisoner (a new PC!), and the encounter begins again with a new victim.

\newRule{End of Scene Regeneration}{
  Each time the party enter a new situation (often when they simply enter a new room), they regain 40\% of their base Fate Points (FP).
  Those with a base of 5 FP regain 2, and those with a base of 10 regain 4.

  Those with Mana Points (MP) can also regain 2 MP plus their Wits Bonus.
...
\iftoggle{oneshot}{%
  ...
  mean regeneration.
  As the PCs enter this new room, they heal 40\% of their \textit{Base FP}, so characters with 5 FP regain 2, and characters with 10 FP regain 4.
  Any characters with Mana Points (MP) regain 2 + their Wits Bonus.
}{}
...
The rungs which are a multiple of seven (seven, fourteen, twenty-one) are set to break once anyone with 7+ HP puts their weight on it.
...
\newRule{Damage Resistance \& Vitals Shots}{

  Damage Resistance (DR) reduce Damage taken due to armour, or just a creature's thick hide.
  The nura rat has `{\scshape DR 2}', so it removes 2 from any Damage Taken.
  ...
  If anyone hits 5 points over what they need to hit the rat, they get a `\textit{Vitals Shot}', which ignores the DR entirely, as the attack hits a sensitive area.
}
...
\paragraph{If the PCs wait patiently and silently,}
...
\paragraph{If the PCs make a sound},
...
\begin{itemize}
  ...
  \item
  The rat attacks the PCs immediately.
  \iftoggle{oneshot}{(As before, players roll 2D6 + Dexterity + Combat roll, at TN equal to the creature's `{\scshape Att}', and the winner deals Damage)}{}
  \item
  The goblin nuramancers observe them for a moment, then flee to the life (room \ref{lift}). 
\end{itemize}

\paragraph{If the PCs enter combat,}
...
\nurarat[\npc{\N\A}{Nura Rat}]
  ...
  \paragraph{Once the PCs leave the room,}
...
\paragraph{If the PCs try to sneak past,}
...
If a single PC fails, everyone fails.}{}%
\iftoggle{core}{%
...
\paragraph{If the PCs corner the goblins,}
...
\iftoggle{oneshot}{
  \begin{itemize}
    ...
    \item
    Anyone with Invocation 2 or more can cast a fireball by spending 3 AP and 2 AP.
    \begin{itemize}
      \item
      Players must roll to avoid the magical flames at TN 7 plus the caster's Intelligence + Projectiles.
      \item
      Players can spend 1 AP to \textit{Keep Edgy} against missile attacks such as fireballs, for the rest of combat.
...
Once the spell is cast, all creatures with 11 HP or fewer in the surrounding 3 areas, instantly raise from the dead as ghouls.
...
\iftoggle{oneshot}{
  \paragraph{If the PCs try to examine a scroll,}
  ...
  Every time players spend a Story Point, the character gains 1 XP for every Story Point marked off so far (so the second gives 2 XP, the third gives 3, and so on).%
...
\paragraph{If the PCs want to identify what a scroll does,}
have them roll Intelligence + Academics, TN 12.

\paragraph{If the PCs want to activate a Portal Scroll,}
...
the PCs head towards the blinding light.
...
\paragraph{If the PCs decide to stop and rest,}
\iftoggle{hardcore}{%
  they are immediately interrupted by a returning raiding party of nura, with fresh prisoners.
}{
  they will be able to recovery for a while,\footnote{As usual, they recover Fatigue Points equal to half their current HP, and 40\% of their base FP.} before a raiding party descends with freshly captured prisoners.
}
...
\paragraph{If any PCs have died,}
this is a good time to give out new characters from the recently captured prisoners.

\paragraph{If the PCs try to hide,}
they will find it easy.

\paragraph{If they try to ambush the nura,}
give them a Wits + Stealth (TN 8) roll to determine the best location to stand.

Success means they can each make one attack roll at TN 7 (not the creature's usual attack score) with +2 Damage.
...
the player can roll Intelligence + Academics, TN 12, as long as they speak Gnomish.
...
\iftoggle{oneshot}{
  ...
  don't do much at first, but every Fatigue Point above the character's \emph{current} HP gives a -1 penalty.
  So a character with 6 Fatigue Points who gets reduced to 3 HP would take a -3 penalty to all actions.
}{}

\iftoggle{oneshot}{
  \paragraph{Staying to stare at the portal}
  counts as a new area, and therefore a new scene, so the PCs can regenerate 40\% of their FP and some MP again.
}{}

\mapentry[kitchen]{Kitchen}

\paragraph{If the PCs have avoided making a lot of noise nearby,}
...
\paragraph{If the PCs try to sneak in,}
have them make a Group Roll of Dexterity + Stealth, TN 8.
\iftoggle{oneshot}{%
...
\iftoggle{oneshot}{
  ...
  The Morale check only uses the Combat Skill at TN 5, with some modifiers.

  \begin{itemize}
    \item
    -2 if any PC is larger has a higher Strength than the goblin.
    \item
    -2 if there are more PCs \emph{in the room} than goblins.
    \item
    -2 if the goblin is wounded.
    \item
    -1 if any PC has displayed magical powers.
  \end{itemize}
  ...
  they attempt to flee into the prison cell the PCs just came from.
  ...
  If the PCs outnumber the goblins, and one is stronger than them, then their total would become `5' and `6'.
...
\newRule{Weapons}{
 \begin{itemize}
    ...
    \item
    The \textit{AP Cost} shows how many AP the character must spend to use the weapon (no matter who instigated the attack).
    \begin{itemize}
      \item
      Characters cannot declare actions when they have less than 1 AP left, but they can (and must) still spend AP to defend themselves.
      \item
      Characters can get pushed into negative AP when they are forced to Defend themselves, which then become a penalty (being on -2 AP means a -2 penalty to all actions).
    \end{itemize}
    \item
    The \textit{Weight Rating} tells you what level of Strength Bonus you need to use the weapon unencumbered.
    If a weapon's Weight Rating is higher than the character's Strength penalty, then every additional point gives them -1 AP at the start of each combat round, and +1 Fatigue Points at the end of combat.
  \end{itemize}
...
\begin{nametable}[YXYYYY]{Kitchen Weapons}
  ...
  \textbf{No.} & \textbf{Name} & \textbf{Attack Bonus} & \textbf{Damage Bonus} & \textbf{AP Cost} & \textbf{Weight Rating} \\\hline
...
\iftoggle{oneshot}{
  ...
  These weapons may not be good quality, but they can still improve the PCs' situation immensely.
...
\iftoggle{oneshot}{
  ...
  in the kitchen regenerates a number of Fatigue Points equal to half the player's current HP (rounded up).
  And of course, at the end of the scene each character also regenerates 40\% of their FP.
}{}
...
\paragraph{If the PCs loot the bodies}
...
he blesses them all with a \textit{Wide Blessing} (which heals $2D6$ FP), and casts a \textit{Fortune} spell on the weakest member, allowing them to receive +1 to their Combat Skill for
\iftoggle{hardcore}{%
  three scenes.
}{
...
\iftoggle{oneshot}{
  \paragraph{If the PCs attack the dragon,}
  he kills the first to attack.

  His {\scshape DR 5} means he reduces all Damage by 5, unless the attacker hits 5 over the TN to attack him (a total of 15), achieving a `Vitals Shot'.
}{}
...
\newRule{Projectiles}{
  ...
  The basic TN is 7, but can go up for targets who are farther away.

  If the enemy decide to \textit{Keep Edgy} (watching out for missile attacks), they can add their Speed + Vigilance to the TN.
  This costs 1 AP, as they need to spend time looking around the room to see where archers (or spellcasters) are.

  So a goblin with Speed 2, Vigilance 1, would be TN 7 to hit normally, but once they spend 1 AP to Keep Edgy, their TN would be 10.
...
\iftoggle{oneshot}{
  Anyone following Laiqu\"e who successfully rescues a child from the warren gains 3 XP at the end of the session.
}{}
...
\iftoggle{oneshot}{
  Characters can only carry a Weight Rating equal or less than their Strength Bonus, or they will take penalties to their AP, and gain Fatigue Points at the end of each scene of activity.

  \paragraph{If the PCs rest,}
  Remember to have them heal Fatigue Points equal to half their HP.
}{}
...
\paragraph{If the PCs have come from room \ref{slugHall},}
...
\paragraph{If the PCs have come up from room \ref{entrycell},}
...
\paragraph{If any of the PCs attempt to sneak in,}
have them roll Dexterity + Stealth, TN 8.
...
Each item taken increases the roll's TN by 1, so taking 3 items would mean a TN of 11.
The player should not roll again -- the original roll remains, but increasing the TN may well turn success into awful failure.
...
\iftoggle{oneshot}{% This weapon chart goes next to the armoury
  ...
  \textbf{Quantity} & \textbf{Name} & \textbf{Attack Bonus} & \textbf{Damage Bonus} & \textbf{AP Cost} & \textbf{Weight Rating} \\\hline
...
\iftoggle{oneshot}{
  ...
  They require only 2 AP to use in combat, but add a +2 Bonus.

  \paragraph{The crossbow}
  (if repaired) deals $1D6+3$ Damage, but requires at least 4 rounds to reload.

  \paragraph{Shortbows}
  Require 1 AP to loose an arrow, and 1 AP to reload.
...
\iftoggle{oneshot}{%
  Each character's Weight Rating is equal to their HP.
}{}
