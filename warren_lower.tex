...
\iftoggle{hardcore}{}{
  ...
  \noindent
  Here the \glspl{pc} awaken in total darkness, bound with ropes, among ragged sheets and small, splintered beds.%
  \footnote{Many gnomes slept in this room, due to the excellent ventilation (room \vref{escapeShaft}).}
  The door is locked.%
  \footnote{Gnomes have a cultural tradition of putting locks on things, especially unexpected things.}
  ...
  \begin{boxtext}
    heavy footsteps pad down a hall, beyond the door.
    An argument begins, then goes silent in the same instant.
    The feet descend with a softer tapping sound from something metal.
    Behind her, \pgls{ogre} stoops to the height of a man to avoid the low ceiling.
  \end{boxtext}

  A goblin druid has come with \pgls{ogre} and the doorkey.
\index{Key!\Glsfmttext{warren}~\nameref{entrycell}}
  The players have time for \pgls{action} each (one dice-roll) before the door opens, and the \gls{ogre} reaches out to take a character to the kitchen (room~\vref{kitchen}).
  ...
  \iftoggle{intro}{%
    ...
    This gives the \glspl{pc} a bonus to any attack roll equal to their \roll{Wits}{Vigilance} +3 (the \gls{ogre} cannot co\"ordinate well in the dark).
  }{}
...
\iftoggle{hardcore}{}{
  \begin{boxtext}
    The air has a subtle draft in the alcove.
    The stone walls have nothing but shallow, empty, shelves on the left hand side, from top to bottom.
  \end{boxtext}

  The chimney's ladder leads to an alcove beside room~\vref{workshop}, \nameref{workshop},%
  \footnote{The gnomes tunnelled down this shaft for mining, then made a ladder to easily reach the lower levels.
  More recently, the alchemical lift (room \ref{lift}) made it redundant, so they've placed a forge in front of the entrance to the workshop (room \ref{workshop}), since this shaft ascends above, as a chimney.}
  where a forge blocks entrance.
  ...
  Failure means alerting, who take it as a sign that lunch is ready.
...
\begin{boxtext}
  ...
  Two goblins lie on top of the cookers, one rests their head on a cloth bag,%
  \footnote{The bag contains the last salt in the kitchen, as the rest has been used to contain the giant slugs in room \vref{slugHall}.
  The bag has \pgls{weight} of~1.}
  while the other cradles their own fat, little, belly as it breathes gently in and out.
...
\iftoggle{intro}{
  ...
  Find the \glspl{weapon}' table \vpageref{weaponsTable}, and tell each player the stats for anything they pick up.
 }{
...
\iftoggle{intro}{}{
  \iftoggle{hardcore}{}{
    \mapentry[sneakingGoblin]{Backstabber}

    One goblin was taking a nap when the \glspl{pc} got out.
    Feeling brave, the little warrior decides it will wait until the \glspl{pc} are not paying attention, stab one in the back, or throw something at them from the shadows, and flee.
    `\nameref{lift}' gives it cover at all times, as it can always move to the opposite side.

    \goblin[\npc{\M\N}{Sneaker}]

    \act{hear the feet in the shadows}{Wits}{Vigilance}{\roll{Dexterity}{Stealth}}
...
The dragon%
\footnote{When the horde attacked, one gnome decided to activate \pgls{deep} scroll in order to flee.
...
And just like the horde, he cannot leave, as he does not know how to read the \Gls{deep} Scrolls, but can guess the answer to any riddle.}
will happily talk with anyone who approaches%
